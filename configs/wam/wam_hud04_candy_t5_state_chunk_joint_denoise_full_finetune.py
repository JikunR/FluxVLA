...
#
# WAM on HUD04 / VCube candy data with state-action and pixel prediction.
# The video and action expert slots are both noised and denoised together in
# one bidirectional MoT pass via the ``joint`` mode. The action expert predicts
# the concatenated ``[state_chunks | actions]`` extended action, while the
# video expert predicts future pixels.
#
    ...
    vla_head=dict(
        ...
        state_expert=dict(
            ...
            config=dict(
                ...
                # and action slices for inference.
        ...
        loss=dict(
            lambda_video=0.0,
            lambda_action=0.0,
            lambda_forward_video=0.0,
            lambda_idm_state_action=0.0,
            lambda_policy_state_action=0.0,
            lambda_joint_video=1.0,
            lambda_joint_state_action=1.0,
