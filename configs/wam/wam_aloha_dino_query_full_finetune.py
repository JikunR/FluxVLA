...
#
# WAM ALOHA fold-cloth training with semantic query DINO supervision.
...
_dino_checkpoint = os.path.join(
    _ckpt_root, 'vit_large_patch14_reg4_dinov2.lvd142m/model.safetensors')
...
model = dict(
    ...
    semantic_feature_extractor=dict(
        type='DinoViTBackbone',
        vision_backbone_id='dino',
        pretrained=True,
        pretrained_cfg=dict(file=_dino_checkpoint),
        img_size=224,
        feature_layer_offset=2,
    ),
    vla_head=dict(
        type='WAMHead',
        video_expert=dict(
        ...
        loss=dict(
            ...
            lambda_forward_semantic=1.0,
            lambda_joint_semantic=0.0,
        ),
...
inference_model = dict(
    model, vlm_backbone=None, semantic_feature_extractor=None)
