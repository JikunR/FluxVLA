...
#
# Cosmos3-Nano full finetune on LIBERO-Spatial benchmark with current state
# prepended as action conditioning.
#
# Usage (8 GPU):
#   torchrun --nproc-per-node=8 scripts/train.py \
#     --config \
#     configs/cosmos3/cosmos3nano_libero_spatial_prepend_state_full_finetune.py
#
# LIBERO-specific settings:
# * LIBERO: action_dim=10 (absolute xyz + rot6d + gripper),
...
# * 128x128 image resolution (same as other VLAs on LIBERO for fair
#   comparison).
...
# LIBERO robot spec
_action_dim = 10  # absolute eef_pos(3) + eef_rot6d(6) + gripper(1)
...
_transforms = [
    ...
    # SimpleNormalizeImages: scales [0,255] uint8 to [-1,1] float32
    # (required by Wan2.2 VAE)
    dict(type='SimpleNormalizeImages'),
    dict(
        type='LiberoDeltaActionToGoalRot6D',
        state_key='states',
        action_key='actions',
        state_window_key='state_window',
    ),
    dict(
...
train_dataloader = dict(
    ...
    dataset=dict(
        ...
        datasets=dict(
            ...
            action_key='action',
            state_window_key='observation.state',
            use_delta=False,
            statistic_name='libero_spatial_no_noops',
            window_start_idx=0,
            frame_window_size=_frame_window_size,
...
eval = dict(
    ...
    dataset=dict(
        ...
        transforms=[
            ...
            dict(
                type='LiberoProprioRot6DFromInputs',
                state_dim=_max_state_dim,
    ...
    denormalize_action=dict(
        type='DenormalizeLiberoRot6DAction',
