...
class WAMHeadTest(unittest.TestCase):
    ...
    def test_wam_mode_ids_are_shared(self):
        self.assertEqual(WAM_TRAINING_MODES,
                         ('forward', 'idm', 'policy', 'joint', 'vgm_policy'))
        self.assertEqual(
...
class WAMConfigTest(unittest.TestCase):
    ...
    def test_canonical_configs_are_wam_only(self):
        for path, suite in (
            ...
            self.assertIsNone(cfg.inference_model.vlm_backbone)
            ...
            if suite == 'libero_10':
                self.assertEqual(
                    dict(cfg.model.vla_head.mode_probs), {
                        'forward': 1.0,
                        'idm': 1.0,
                        'policy': 1.0,
                        'joint': 0.0,
            ...
            self.assertEqual(
                dict(cfg.runner.collator.mode_probs), {
                    'forward': 1.0,
                    'idm': 1.0,
                    'policy': 1.0,
                    'joint': 0.0,
            ...
            eval_prompt_transforms = [
                ...
                if transform.type == 'LoadCachedTextEmbedding'
            ]
