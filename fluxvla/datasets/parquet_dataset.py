    ...
    def __init__(self,
        ...
        for transform in transforms:
            ...
            if (inject_model_path
                    and transform.get('type') != 'LoadCachedTextEmbedding'):
    ...
    def __call__(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'context' in inputs or 'context_mask' in inputs:
            if 'context' not in inputs or 'context_mask' not in inputs:
                raise KeyError(
                    '`context` and `context_mask` must be provided together.')
            batch['context'] = _batched_cuda_tensor(inputs['context'])
            batch['context_mask'] = _batched_cuda_tensor(
                inputs['context_mask'])
        else:
            batch['lang_tokens'] = _batched_cuda_tensor(
                inputs['lang_tokens'])
            batch['lang_masks'] = _batched_cuda_tensor(inputs['lang_masks'])
