...
def _build_uninitialized(factory, device: str, torch_dtype: torch.dtype):
    """Construct directly on meta for low-memory checkpoint loading."""
    if torch.device(device).type == 'meta':
        with torch.device('meta'):
            return factory().to(dtype=torch_dtype)
    return factory().to(device=device, dtype=torch_dtype)
...
def build_wan_text_encoder(
    ...
    if skip_load_from_pretrain:
        return _build_uninitialized(WanTextEncoder, device, torch_dtype)
...
def build_wan_video_vae38(
    ...
    if skip_load_from_pretrain:
        return _build_uninitialized(WanVideoVAE38, device, torch_dtype)
    if checkpoint_root is None:
        raise ValueError(
...
def build_wan_video_dit(
    ...
    if skip_load_from_pretrain:
        return _build_uninitialized(lambda: WanVideoDiT(**validated_config),
                                    device, torch_dtype)
    if checkpoint_root is None:
        raise ValueError(
...
def build_action_dit(
    ...
    model = _build_uninitialized(lambda: ActionDiT(**config), device,
                                 torch_dtype)
    if skip_load_from_pretrain or pretrained_path is None:
        return model
    ...
    if not path.is_file():
        raise FileNotFoundError(f'ActionDiT checkpoint does not exist: {path}')

    payload = torch.load(path, map_location='cpu', weights_only=True)
    if not isinstance(payload, dict):
        raise TypeError(f'ActionDiT checkpoint must contain a dict, got '
    ...
    if mismatched_meta:
        raise ValueError(
            f'ActionDiT checkpoint metadata does not match config: '
            f'{mismatched_meta}')

    backbone_state = payload.get('backbone_state_dict')
    if not isinstance(backbone_state, dict):
        raise TypeError(
            '`backbone_state_dict` must be a dict in the ActionDiT checkpoint.'
        )
    model_state = model.state_dict()
    backbone_keys = {
        key
        for key in model_state
        if not key.startswith(('action_encoder.', 'head.'))
    }
    if set(backbone_state) != backbone_keys:
        missing = sorted(backbone_keys - set(backbone_state))
        unexpected = sorted(set(backbone_state) - backbone_keys)
        raise ValueError(
            'ActionDiT checkpoint keys do not match the model backbone: '
            f'missing={missing[:10]}, unexpected={unexpected[:10]}')
    for key, value in backbone_state.items():
        target = model_state[key]
        if value.shape != target.shape:
            raise ValueError(
                f'ActionDiT checkpoint shape mismatch for {key!r}: '
                f'{tuple(value.shape)} != {tuple(target.shape)}')
        model_state[key] = value.to(device=target.device, dtype=target.dtype)
