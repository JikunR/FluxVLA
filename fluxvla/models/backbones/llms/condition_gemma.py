...
class GemmaRMSNorm(nn.Module):
    """
    RMSNorm module for Gemma model.
    ...
    def forward(self, x, cond=None):
        """Forward pass for GemmaRMSNorm.

        Args:
            x (`torch.Tensor` of shape
               `(batch_size, seq_length, hidden_size)`):
                The input tensor to normalize.
            cond (`torch.Tensor` of shape
                  `(batch_size, cond_dim)`, *optional*):
                The condition tensor to modulate the normalization.

        Returns:
            tuple: A tuple containing the normalized input tensor
                and the gate tensor.
                - normed_inputs (`torch.Tensor` of shape
                  `(batch_size, seq_length, hidden_size)`):
                    The normalized input tensor.
                - gate (`torch.Tensor` of shape
                       `(batch_size, seq_length, 1)`, *optional*):
                    The gate tensor.
                    If cond is None or self.dense is None,
                    the gate tensor is None.
                    If cond is provided and self.dense is not None,
                    the gate tensor is a scalar tensor.
        """
        ...
        if cond is None or self.dense is None:
            # regular RMSNorm
            # scale by learned parameter in float32
        ...
        # use model weight dtype to ensure compatibility
        # model_dtype = self.dense.weight.dtype
        # Use the model's dtype (bfloat16)
        ...
        normed_inputs = (
            normed_inputs * (1 + scale.to(torch.float32)) +
...
class GemmaMLP(nn.Module):
    ...
    Args:
        config (GemmaConfig): The configuration of the MLP.
    """
    ...
    def forward(self, x):
        ...
        Returns:
            down_proj (`torch.Tensor` of shape
                      `(batch_size, seq_length, hidden_size)`):
...
class GemmaRotaryEmbedding(nn.Module):
    """
    Rotary Embedding module for Gemma model.

    Args:
        config (GemmaConfig): The configuration of the Rotary Embedding.
    ...
    def forward(self, x, position_ids):
        """Forward pass for GemmaRotaryEmbedding.

        Args:
            x (`torch.Tensor` of shape
               `(batch_size, seq_length, hidden_size)`):
                The input tensor to the Rotary Embedding.
            position_ids (`torch.Tensor` of shape
                          `(batch_size, seq_length)`):
                The position ids to the Rotary Embedding.

        Returns:
            tuple: A tuple containing the cosine and sine tensors.
                - cos (`torch.Tensor` of shape
                  `(batch_size, seq_length, hidden_size)`):
                    The cosine tensor.
                - sin (`torch.Tensor` of shape
                       `(batch_size, seq_length, hidden_size)`):
                    The sine tensor.
        """
        ...
        with torch.autocast(
                device_type=device_type, enabled=False):  # Force float32
...
def apply_rotary_pos_emb(q, k, cos, sin, position_ids=None, unsqueeze_dim=1):
    ...
    Args:
        ...
        unsqueeze_dim (`int`, *optional*, defaults to 1):
            ...
            can be properly broadcasted to the dimensions of q and k.
            ...
            Similarly, if q and k have the shape
...
class GemmaAttention(nn.Module):
    ...
    Returns:
        tuple: A tuple containing the attention output
            and the attention weights.
            - attn_output (`torch.Tensor` of shape
              `(batch_size, seq_length, hidden_size)`):
                The output tensor of the attention.
            - attn_weights (`torch.Tensor` of shape
                           `(batch_size, seq_length, seq_length)`):
                The attention weights.

    Raises:
        ValueError: If the input tensor is not of shape
            `(batch_size, seq_length, hidden_size)`.
        ValueError: If the position ids are not of shape
            `(batch_size, seq_length)`.
        ValueError: If the attention mask is not of shape
            `(batch_size, seq_length, seq_length)`.
        ValueError: If the past key value is not of shape
            `(batch_size, seq_length, hidden_size)`.
        ValueError: If the cache position is not of shape
            `(batch_size, seq_length)`.
        ValueError: If the use cache is not a boolean.
    """
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        """Forward pass for GemmaAttention.

        Args:
            hidden_states (`torch.Tensor` of shape
                           `(batch_size, seq_length, hidden_size)`):
                The input tensor to the attention.
            position_embeddings (tuple[torch.Tensor, torch.Tensor]):
                A tuple containing the cosine and sine tensors.
            attention_mask (`torch.Tensor` of shape
                           `(batch_size, seq_length, seq_length)`,
                           *optional*):
                The attention mask.
...
class GemmaDecoderLayer(GradientCheckpointingLayer):
    ...
    Args:
        config (GemmaConfig): The configuration of the decoder layer.
        layer_idx (int): The index of the layer.
    """
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
                                                 ...
                                                 torch.FloatTensor]]]:
        """Forward pass for GemmaDecoderLayer.
        ...
        Args:
            ...
            position_embeddings (tuple[torch.Tensor, torch.Tensor]):
                A tuple containing the cosine and sine tensors.
            adarms_cond (`torch.Tensor` of shape
                        `(batch_size, cond_dim)`, *optional*):
                The condition tensor for ADARMS.
            **kwargs: Additional keyword arguments.

        Returns:
            tuple: A tuple containing the hidden states
                and the attention weights.
                - hidden_states (`torch.Tensor` of shape
                  `(batch_size, seq_length, hidden_size)`):
                    The hidden states.
        ...
        Raises:
            ...
            ValueError: If the cache position is not of shape
                `(batch_size, seq_length)`.
            ValueError: If the use cache is not a boolean.
            ValueError: If the output attentions is not a boolean.
            ValueError: If the adarms cond is not of shape
                `(batch_size, cond_dim)`.
            ValueError: If the position embeddings are not a tuple
                of `(torch.Tensor, torch.Tensor)`.
...
class ConditionGemmaModel(GemmaPreTrainedModel):
    """
    Condition Gemma model.
    ...
    Args:
        ...
        hidden_activation (`str`, *optional*, defaults to
            `"gelu_pytorch_tanh"`):
            The activation function for the hidden layer (alias for
            hidden_act).
        hidden_size (`int`, *optional*, defaults to `1024`):
            The dimension of the hidden layer.
        initializer_range (`float`, *optional*, defaults to `0.02`):
            The range of the initializer for weight initialization.
        intermediate_size (`int`, *optional*, defaults to `4096`):
            The dimension of the intermediate layer in the MLP.
        max_position_embeddings (`int`, *optional*, defaults to `8192`):
            The maximum number of position embeddings.
        model_type (`str`, *optional*, defaults to `"gemma"`):
        ...
        rope_theta (`float`, *optional*, defaults to `10000.0`):
            The theta parameter for Rotary Position Embedding (RoPE).
        torch_dtype (`str`, *optional*, defaults to `"float32"`):
            The dtype of the torch tensors.
        transformers_version (`str`, *optional*, defaults to `"4.48.1"`):
            The version of the transformers library.
        use_adarms (`bool`, *optional*, defaults to `True`):
            Whether to use ADARMS (Adaptive RMS Normalization).
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        """Forward pass for ConditionGemmaModel.
        ...
        Raises:
            ValueError: If the input ids is not specified
                and the input embeddings are not specified.
            ValueError: If the input ids is specified
                and the input embeddings are specified.
            ValueError: If the use cache is not a boolean.
            ValueError: If the output attentions is not a boolean.
            ValueError: If the output hidden states is not a boolean.
            ValueError: If the cache position is not
                a torch.LongTensor.
            ValueError: If the adarms cond is not a torch.Tensor.
            ValueError: If the kwargs are not a dictionary.
        """
        output_attentions = (
        ...
        if len(self.layers) > 0 and self.layers[
        ...
        # normalized
        # Gemma downcasts the below to float16,
        # causing sqrt(3072)=55.4256 to become 55.5
...
class GemmaForCausalLM(GemmaPreTrainedModel, GenerationMixin):
    """GemmaForCausalLM is a model for causal language modeling.

    Args:
        config (GemmaConfig): The configuration of the Gemma model.

    Returns:
        GemmaForCausalLM: The model.
    """
...
@auto_docstring(custom_intro="""
    The Gemma Model transformer with a sequence classification head on top
    (linear layer).

    [`GemmaForSequenceClassification`] uses the last token in order to do
    the classification, as other causal models (e.g. GPT-2) do.
    ...
    each row. If no `pad_token_id` is defined, it simply takes the last
    value in each row of the batch. Since it cannot guess the padding tokens
    when `inputs_embeds` are passed instead of `input_ids`, it does the same
    (take the last value in each row of the batch).
    """)
class GemmaForSequenceClassification(GemmaPreTrainedModel):
    """GemmaForSequenceClassification is a model for sequence classification.

    Args:
        config (GemmaConfig): The configuration of the Gemma model.

    Returns:
        GemmaForSequenceClassification: The model.
    """
    ...
    def forward(
        ...
        if self.config.pad_token_id is None and batch_size != 1:
            raise ValueError(
                'Cannot handle batch sizes > 1 if no padding token is '
                'defined.')
        if self.config.pad_token_id is None:
            last_non_pad_token = -1
        elif input_ids is not None:
            # To handle both left- and right- padding, we take the rightmost
            # token that is not equal to pad_token_id
...
class GemmaForTokenClassification(GemmaPreTrainedModel):
    """GemmaForTokenClassification is a model for token classification.

    Args:
        config (GemmaConfig): The configuration of the Gemma model.

    Returns:
        GemmaForTokenClassification: The model.
    """
