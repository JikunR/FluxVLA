...
def adjust_by_factor(
        number: int,
        factor: int,
        method: Literal['round', 'ceil', 'floor'] = 'round') -> int:
...
def to_rgb(pil_image: Image.Image) -> Image.Image:
    if pil_image.mode == 'RGBA':
        ...
        white_background.paste(
            pil_image, mask=pil_image.split()[3])  # Use alpha channel as mask
...
def _read_video_torchvision(ele: dict, ) -> (torch.Tensor, float, list):
...
def _read_video_decord(ele: dict, ) -> (torch.Tensor, float, list):
...
def fetch_video(
    ...
    if isinstance(ele['video'], str):
        ...
        try:
            video, sample_fps, timestamps = VIDEO_READER_BACKENDS[
...
class Eagle2_5_VLProcessor(ProcessorMixin):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.image_token = (
            ...
            if hasattr(tokenizer, 'image_token') else image_token)
        self.video_token = (
            tokenizer.video_token
            if hasattr(tokenizer, 'video_token') else video_token)
        self.image_token_id = (
            tokenizer.image_token_id
            if getattr(tokenizer, 'image_token_id', None) else
            tokenizer.convert_tokens_to_ids(self.image_token))
        self.video_token_id = (
            tokenizer.video_token_id
            if getattr(tokenizer, 'video_token_id', None) else
    ...
    def replace_media_placeholder(self, text, image_list, video_list,
                                  timestamps_list, fps_list, **output_kwargs):
        ...
        def replace_in_text(text):
            ...
            def repl(match):
                ...
                idx_in_list = int(
                    match.group(2)) - 1  # Convert to list index (0-based)
                ...
                if media_type == 'image':
                    ...
                    sampled_fps = fps_list[
                        idx_in_list] if fps_list is not None else None
                    ...
                    if frame_timestamps is not None:
                        ...
                        special_placeholder = [
                            ...
                            for i, num_of_tiles in enumerate(
                                num_of_tiles_each_frame)
                        ]
                    else:
                        special_placeholder = [
        ...
        **kwargs: Unpack[Eagle2_5_VLProcessorKwargs],
        ...
        for sample in text_list:
            timestamps_list = (
                ...
                if timestamps_batch is not None else None)
            fps_list = fps_batch[
                video_start_idx:] if fps_batch is not None else None
            (
                sample,
                pixel_values,
                image_sizes,
        ...
        if len(pixel_values_list) > 0:
            ...
            image_inputs = {}
        video_inputs = {}
        text_inputs = self.tokenizer(new_sample_list,
    ...
    def get_number_tiles_based_on_image_size(self, image_size: tuple,
        ...
        if return_video_kwargs:
            return (
                ...
                video_inputs,
                {
                    ...
                    'timestamps': video_timestamps_list
                },
            )
        return image_inputs, video_inputs

    def extract_vision_info(
            self, conversations: list[dict] | list[list[dict]]) -> list[dict]:
        vision_infos = []
                ...
                if isinstance(message['content'], list):
                    for ele in message['content']:
                        if ('image' in ele or 'image_url' in ele
                                or 'video' in ele or ele['type']
                                in ('image', 'image_url', 'video')):
                            vision_infos.append(ele)
        return vision_infos

    def py_apply_chat_template(self,
                               messages,
                               tokenize=False,
    ...
    @classmethod
    def from_args_and_dict(cls, args, processor_dict: dict[str, Any],
                           **kwargs):
        """
