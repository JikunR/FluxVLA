...
from transformers.image_utils import (ChannelDimension, ImageInput,
                                      PILImageResampling, SizeDict, VideoInput,
                                      get_image_size, make_flat_list_of_images,
                                      validate_kwargs)
...
def crop(img: torch.Tensor, left: int, top: int, right: int,
         bottom: int) -> torch.Tensor:
    ...
    if img.ndim not in [2, 3]:
        raise ValueError('Image should have 2 or 3 dimensions. Got {}'.format(
            img.ndim))
...
class Eagle2_5_VLImageProcessorFast(BaseImageProcessorFast):
    ...
    def _pad_for_patching(
            ...
            input_data_format: ChannelDimension) -> 'torch.Tensor':
        """
        ...
        padded_image = F.pad(
            image, padding=[paste_x, paste_y, paste_x, paste_y])
    ...
    def _get_image_patches(
        ...
        for i in range(blocks):
            ...
            # split the image
            split_img = crop(image_used_to_split, box[0], box[1], box[2],
                             box[3])
    ...
    def _preprocess(
        ...
        for image in images:
            ...
            processed_image_patches = (
                ...
                if return_tensors else processed_image_patches)
        ...
        processed_images = (
            ...
            if return_tensors else processed_images)
        return BatchFeature(
            data={
    ...
    def preprocess(
        ...
        **kwargs: Unpack[Eagle2_5_VLFastImageProcessorKwargs],
        ...
        kwargs['interpolation'] = (
            pil_torch_interpolation_mapping[resample] if isinstance(
                resample, (PILImageResampling, int)) else resample)
