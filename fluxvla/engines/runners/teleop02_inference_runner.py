...
class Teleop02InferenceRunner(BaseInferenceRunner):
    """Runner for Teleop02 (HUD04) loco-mani humanoid robot inference.

    Uses mros middleware instead of rospy. Overrides the main inference
    loop to remove rospy dependency, using time-based rate control.

    The robot has:
        - 1 head camera
        - 33-dim state (31 joints + 2 hand_closed)
        - 41-dim action (4 body poses as xyz+rot6d, base, hands)
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        if 'operator' not in kwargs or kwargs['operator'] is None:
            kwargs['operator'] = {
                ...
                'teleop_cmd_topic': '/teleop_cmd_vla',
    ...
    def run(self, initial_instruction='pour water into the cup'):
        ...
        overwatch.info('Starting Teleop02 inference runner')
    ...
    def get_ros_observation(self):
        """Get observation from Teleop02Operator via mros.
    ...
    def _execute_actions(self, actions: np.ndarray, rate):
        ...
        Args:
            actions (np.ndarray): Array of denormalized 41-dim actions.
    ...
    def _move_to_prepare_pose(self):
        """No-op for Teleop02 (teleop-controlled robot)."""
        pass

    def cleanup(self):
        """Clean up resources."""
        print('Cleaning up Teleop02InferenceRunner')
        self._running = False
        super().cleanup()
